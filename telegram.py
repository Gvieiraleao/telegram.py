...
class Telegram(plugins.Plugin):
    ...
    def _read_wpa_sec_file(self):
        wpa_sec_filepath = '/home/pi/handshakes/wpa-sec.cracked.potfile'
        try:
            ...
            try:
                ...
                if 1 <= selected_number <= len(os.listdir(self.qrcode_dir)):
                    ...
                    with open(f"{self.qrcode_dir}{selected_file}", 'rb') as f:
                        ...
                        geojson_files = glob.glob(f"/home/pi/handshakes/*_{bssid}.gps.json")
                        geojson_files += glob.glob(f"/home/pi/handshakes/*_{bssid}.geo.json")
                        if geojson_files:
    ...
    def handle_exception(self, update, context, e) -> None:
        if update.effective_chat.id == int(self.options.get("chat_id")):
            error_text = f"⛔ Unexpected error ocurred:\n<code>{e}</code>\nIf this keeps happening, please check the logs and submit an issue to https://github.com/gvieiraleao/telegram.py/issues/new/choose"
                ...
                if not os.path.exists("telegram-bot"):
                    ...
                    subprocess.run(
                        [
                            "git",
                            "clone",
                            "https://github.com/gvieiraleao/telegram.py",
                            "telegram-bot",
                        ],
                        check=True,
                    )
    ...
    def read_potfiles_cracked(self, agent, update, context):
        if update.effective_chat.id == int(self.options.get("chat_id")):
            potfiles_dir = "/home/pi/handshakes"
    ...
    def create_pcap_backup(self, agent, update, context):
        if update.effective_chat.id == int(self.options.get("chat_id")):
            context.bot.send_chat_action(
                chat_id=update.effective_chat.id, action="typing"
            )
            
            pcap_folder = "/home/pi/handshake/"
            backup_dir = "/home/pi/"

            # Get datetime
            current_time = datetime.now()
            formatted_time = current_time.strftime("%Y-%m-%d-%H:%M:%S")
            backup_file_name = f"pcap-backup-{formatted_time}.zip"
            backup_zip_path = os.path.join(backup_dir, backup_file_name)

            try:
                # Create a zip file
                subprocess.run(["sudo", "zip", "-r", backup_zip_path, pcap_folder])

                self.generate_log("PCAP backup created successfully.", "DEBUG")

            except Exception as e:
                self.handle_exception(update, context, e)

            # Obtain the file size
            file_size = os.path.getsize(backup_zip_path)
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "📤 Send me the backup here", callback_data="send_pcap_backup"
                    ),
                ],
            ]

            response = f"✅ PCAP backup created successfully in <code>/home/pi</code>.\nFile size: <b>{file_size} MB</b>"
            self.update_existing_message(update, context, response, keyboard)
            self.completed_tasks += 1
            if self.completed_tasks == self.num_tasks:
                self.terminate_program()
            self.last_pcap_backup = backup_file_name

    def send_pcap_backup(self, agent, update, context):
        if update.effective_chat.id == int(self.options.get("chat_id")):
            chat_id = update.effective_user["id"]
            context.bot.send_chat_action(chat_id, "upload_document")

            try:
                backup = self.last_pcap_backup
                if backup:
                    self.generate_log(f"Sending PCAP backup: {backup}", "DEBUG")
                    backup_path = f"/home/pi/{backup}"
                    with open(backup_path, "rb") as backup_file:
                        update.effective_chat.send_document(document=backup_file)
                    update.effective_message.reply_text("PCAP backup sent successfully.")
                else:
                    self.generate_log("No PCAP backup file found.", "ERROR")
                    update.effective_message.reply_text("No PCAP backup file found.")
            except Exception as e:
                self.handle_exception(update, context, e)
